	...
	-   安装第三方模块
		...
		2.	安装第三方模块
			-   pip 工具需要在命令行中运行： 向它传入 install 命令， 跟上想要安装的模块名称。
				...
				安装模块后，可以在交互式环境中运行import ModuleName，测试安装是否成功。
...
*  字符串，
	...
	-   转义字符
		...
		“转义字符”让你输入一些字符，它们用其他方式是不可能放在字符串里的。转义字符包含一个倒斜杠 （\） ，
		 紧跟着是想要添加到字符串中的字符。 （尽管它包含两个字符，但大家公认它是一个转义字符。 ）
...
*  列表，
	-   列表的数据结构。它可以写作中括号之间的一列逗号分隔的值。列表的元素不必是同一类型。
		...
		>>> a[-2:]
		['India', 'Fedora']
		#	有个办法可以很容易地记住切片的工作方式：切片时的索引是在两个元素之间 。左边第一个元素的索引为 0，
	...
	-   引用
		...
		但列表不是这样的。当你将列表赋给一个变量时，实际上是将列表的“引用”赋给了该变量。引用是一个值，指向某些数据。
...
*   作用域
	...
	def spam():
		...
		# print(eggs, end = '\n ' + '=' * 10 + '\n')
		#	发生这个错误是因为， Python 看到 spam()函数中有针对 eggs 的赋值语句， 因此认为 eggs 变量是局部变量。 
...
*   小结
	...
	```
*	正则表达式方法
	-   regexObject = re.compile(r'regexRule')，regexResult = regexObject.search(string)
...
*	调试器
	...
	-	subulimeTextREPL
*	抛出异常
	-	当 Python 试图执行无效代码时，就会抛出异常。在第 3 章中，你已看到如何使用 try 和 except语句来处理Python的异常，这样程序就可以从你预期的异常中恢复。但你也可以在代码中抛出自己的异常。抛出异常相当于是说： “停止运行这个函数中的代码，将程序执行转到 except 语 句 ” 。
	-	抛出异常使用raise语句。raise语句包含以下部分。raise Exception('This is the error massage.')
		1.	raise关键字
		2.	对Exception函数的调用。
		...
		Traceback (most recent call last):
		  File "<console>", line 1, in <module>
		Exception: This is  the error massage.
...
*	获取反向跟踪的字符串
	-	导入traceback模块
	-	tarceback.format_exc()  --  返回错误信息的字符串
	```
	import os, traceback
	def spam():
		bacon()
	def bacon():
		raise Exception('This is an exception massage.')
	 try:
		 spam()
	except:
	...
	```
*	断言
	-	assert语句(assert condition, 'if False, throw this sentenses.')
		1.	assert 关键字
		2. 条件(求值为True or False 的表达式)
		3.	逗号(,)
		4.	当条件为False时，显示的字符串
		```
		>>> podBayDoorStatus = 'open'
		>>> assert podBayDoorStatus == 'open', 'The pod bay doors need to be "open"!'
		>>> podBayDoorStatus = 'I\'m sorry,Dave,I afraid I can\'t do that'
		>>> assert podBayDoorStatus == 'open', 'The pod bay doors need to be open.'
		Traceback (most recent call last):
		  File "<console>", line 1, in <module>
		...
		```
	-	禁用断言。运行python时传入-O参数，可以禁用断言。
*	日志
	-	使用日志模块(factorialLog.py)
	```
	import logging
	...
	|  级别               |  日志函数                |  描述  |
	|   ----               |   ----                       |   ----  |
	|  DEBUG          |  logging.debug()       |  最低级别，用于细节，当诊断问题的时候才会关心这些信息  |
	|  INFO            |  logging.info()          |  用于记录程序一般时间信息                                                 |
	|  WARMING    |  loggging.warning()  |  用于表示可能会出现的问题，但不会导致程序停止运行         |
	|  ERROR          |  logging.error()        |  用于记录错误，它将导致程序做某事失败                             |
	|  CRITICAL     |  logging.critical()     |  致命错误，将导致程序完全停止运行                                    |
	...
	```
	-	禁用日志
		*	logging.disable(level)  -- 传入level参数禁用对应等级及以下的信息， logging.disable(logging.CRITICAL)禁用所有的日志消息
		*	因为 logging.disable() 将禁用它之后的所有消息， 你可能希望将它添加到程序中接近 import logging 代码行的位置。这样就很容易找到它，根据需要注释掉它，或取消注释，从而启用或禁用日志消息。 
	-	将日志信息写入到文件
		*	logging.basicConfig(filename='filename.txt')  --  将日志信息写入到filename.txt
	```
	>>> import logging
	>>> logging.basicConfig(level=logging.INFO, format=' %(asctime)s - %(levelname)s - %(message)s ')
	>>> logging.info('test throw message')
	 2021-04-22 22:33:06,315 - INFO - test throw message 
	>>> logging.debug('test throw message')
	>>> logging.warning('test throw message')
	 2021-04-22 22:34:31,427 - WARNING - test throw message 
	>>> logging.error('This is error message')
	 2021-04-22 22:35:54,654 - ERROR - This is error message 
	>>> logging.critical('Oops, something wrong, please connact the admin')
	 2021-04-22 22:37:03,771 - CRITICAL - Oops, something wrong, please connact the admin 
	>>> logging.disable(logging.CRITICAL)
	>>> logging.error('ERROR!!')
	>>> logging.critical('CRITICAL !!!')
	>>> logging.disable(logging.ERROR)
	>>> logging.critical('CRITICAL !!!')
	 2021-04-22 22:42:00,032 - CRITICAL - CRITICAL !!! 
	>>> logging.error('ERROR!!')
	```
	-	小结
	```
